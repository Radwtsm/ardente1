  ...
  return (
    <Grid margin={0} marginTop={2} className={styles.main_container} container spacing={2} padding={0} onMouseEnter={() => dispatch(isProject())} onMouseLeave={() => dispatch(isDefault())}>
      ...
      <Grid className={styles.first} paddingLeft={0} xs={6} gap={2}>
        <Container style={{padding:0}} className={styles.first_one}><motion.div className={styles.overlay} initial={{ opacity: 0,x:300 }} transition={{ duration: 0.5 }} whileHover={{ opacity: 1, backgroundColor: 'rgba(255,114,0,0.54)',x:0 }}><motion.p className={styles.overlay_text}>TEST1</motion.p></motion.div></Container>
